...
public class UserResource {
	...
	// POST A NEW USER
		...
		public Response updateUser(
				...
				@FormParam("novaPassword") String novaPassword,
				@PathParam("username") String username,
				@FormParam("token") String token,
				@Context UriInfo uriInfo) {
			UserManager userManager = new UserManager();
			
			System.out.println(email+" "+novoEmail+" "+password+" "+novaPassword+" "+username);
			try {
				// Verify JWT token
				...
				// Get user data
				String LoggedUser = (String) Jwts.parser().setSigningKey(userManager.getKey()).parseClaimsJws(token)
						.getBody().get("username");

				userManager.update(email, novoEmail, password, novaPassword, username, LoggedUser);
				return Response.serverError().status(200).type("text/plain").entity("Perfil Editado").build();
			} catch (SignatureException e) {
				System.out.println(e);
				// don't trust the JWT!
				return Response.serverError().status(401).type("text/plain").entity("Não tens permissões!").build();
			}
